...
async def set_reminder_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    keyboard = [
        [InlineKeyboardButton("⏰ На хвилину", callback_data="1_min"),
         InlineKeyboardButton("⏰ На годину", callback_data="1_hour")],
...
async def get_relative_time(update: Update, context: ContextTypes.DEFAULT_TYPE, unit: str) -> int:
    ...
    try:
        ...
        now = get_current_time()
        if unit == "minutes":
            context.user_data['reminder_time'] = now + timedelta(minutes=value)
        elif unit == "hours":
            context.user_data['reminder_time'] = now + timedelta(hours=value)
        elif unit == "days":
            context.user_data['reminder_time'] = now + timedelta(days=value)
...
async def get_minutes(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int: return await get_relative_time(update,
                                                                                                                 context,
                                                                                                                 "minutes")


async def get_hours(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int: return await get_relative_time(update,
                                                                                                               context,
                                                                                                               "hours")


async def get_days(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int: return await get_relative_time(update,
                                                                                                              context,
                                                                                                              "days")
